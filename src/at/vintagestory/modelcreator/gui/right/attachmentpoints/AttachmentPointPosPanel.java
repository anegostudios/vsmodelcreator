...
public class AttachmentPointPosPanel extends JPanel implements IValueUpdater
{
	...
	public AttachmentPointPosPanel()
	{
	...
	public void modifyPosition(EnumAxis axis, int direction, int modifiers) {
		...
		case X:
			point.addPosX(size);			
			xPositionField.setText(df.format(point.getPosX()));
			
			break;
		case Y:
			point.addPosY(size);
			yPositionField.setText(df.format(point.getPosY()));
			break;
		default:
			point.addPosZ(size);
			zPositionField.setText(df.format(point.getPosZ()));
